...
fn migrate(conn: &Connection) -> rusqlite::Result<()> {
    ...
    if current < 8 {
        ...
        ensure_session_column(conn, "has_user_message", "INTEGER NOT NULL DEFAULT 0")?;
        conn.execute(
    ...
    if current < 11 {
        ...
        ensure_session_column(conn, "pinned", "INTEGER NOT NULL DEFAULT 0")?;
        conn.execute_batch(
...
mod tests {
    ...
    #[test]
    fn linked_work_item_survives_upgrade_and_reopen() {
        let path = std::env::temp_dir().join(format!(
            "jayhun-linked-upgrade-{}-{}.db",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
        ...
        let mut row = sample("s1", "/tmp/a", "Fix PR");
        {
            ...
            let conn = store.conn.lock().unwrap();
            upsert_session(&conn, &row).unwrap();
            // Recreate the pre-link schema with an existing conversation.
            conn.execute_batch(
                "DROP INDEX sessions_cwd_cover_idx;
                 ALTER TABLE sessions DROP COLUMN linked_work_item_json;
        ...
        }));
        {
            ...
            assert_eq!(old.blocks, row.blocks);
            assert_eq!(old.linked_work_item, None);
            assert_eq!(list_by_project(&conn, "/tmp/a").unwrap()[0].id, "s1");
            upsert_session(&conn, &row).unwrap();
        }
        {
            let store = SessionStore::open(path.clone()).unwrap();
    ...
    fn list_linked_finds_threads_across_projects() {
        ...
        set_archived(&conn, "s2", true).unwrap();
        let mut ask = sample("ask", "/tmp/b", "Temporary Ask");
        ask.linked_work_item = first.linked_work_item.clone();
        upsert_session(&conn, &ask).unwrap();
        conn.execute("UPDATE sessions SET inbox_ask = '{}' WHERE id = 'ask'", [])
            .unwrap();
        let mut blank = sample("blank", "/tmp/b", "Blank");
        blank.linked_work_item = first.linked_work_item.clone();
        blank.blocks = json!([]);
        ...
        assert!(rows.iter().any(|row| row.id == "s2" && row.archived));
        assert!(rows
            .iter()
            .all(|row| row.linked_work_item == first.linked_work_item));
    }
