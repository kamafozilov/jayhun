...
export const INTERRUPT_MESSAGE =
  "Turn interrupted when Jayhun quit.";
...
export function quitWhileBusyMessage(count: number): string {
  if (count === 1) {
    return "1 chat is still running. Quit anyway? It will resume when you reopen Jayhun.";
  }
  return `${count} chats are still running. Quit anyway? They will resume when you reopen Jayhun.`;
}
