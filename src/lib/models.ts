...
export function resolveModel(harness: HarnessId, id?: string): AgentModel {
  ...
  if (id) {
    ...
    // A built-in list cannot reject a model saved from a live CLI catalog.
    // Keep its ID until this provider has answered the catalog request.
    const owner = id.includes(":") ? id.slice(0, id.indexOf(":")) : undefined;
    if (!hasLiveCatalog(harness) && (!owner || owner === harness)) {
      return { id, harness, name: nativeIdFrom(id), nativeId: nativeIdFrom(id) };
    }
  ...
  return (
    ...
    MODELS.find((model) => model.harness === harness) ?? {
      ...
      nativeId: nativeIdFrom(fallbackId),
    }
  );
}
