...
async function ensureLive(input: HarnessSessionInput): Promise<Live> {
  ...
  watchChild(
    ...
    (line) => {
      console.debug("[jayhun] grok stderr", line);
  ...
  try {
    ...
    if (methodId) {
      await acp
        ...
        .catch((error: unknown) => {
          console.debug("[jayhun] grok authenticate", error);
        });
    }
...
function ignoreUnsupportedControl(method: string, error: unknown): void {
  console.debug(`[jayhun] grok ${method} failed`, error);
  const detail = error instanceof Error ? error.message : String(error);
...
async function handleRequest(
  ...
  if (method === "_x.ai/exit_plan_mode" || method === "x.ai/exit_plan_mode") {
    ...
    await live.acp
      ...
      // Jayhun's separate Build turn is the only approval boundary.
