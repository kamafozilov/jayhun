...
export function refreshOpenCodeCatalog(): Promise<void> {
  ...
  inflight = discoverOpenCodeModels()
    ...
    .catch((error: unknown) => {
      console.debug("[jayhun] opencode catalog", error);
    })
    .finally(() => {
      inflight = null;
    });
...
async function discoverOpenCodeModels(): Promise<AgentModel[]> {
  ...
  if (!version) {
    throw new Error(
      `Unable to determine OpenCode version. Jayhun requires v${MINIMUM_OPENCODE_VERSION} or newer.`,
    );
  }
  ...
  try {
    ...
    console.debug("[jayhun] opencode agents", error);
  }
